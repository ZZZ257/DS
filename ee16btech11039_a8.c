...
deltree(&root);// final deallocation
free(nil);
return 0;
...
void fixinsert(struct node ** root, struct node * x)
{
	...
	else
	{
		...
		else//left child of right child
		{
			x=x->parent;
			rrot(x,root);
			fixinsert(root,x);
...
struct node * search(struct node *x,int n, int p)// p=0 indicates that address of node that contains list n must be returned p=1 indicates that path must be printed 
{
...
struct node * succ(int a, struct node *y)
{
	...
	while(y!=NULL)
		...
		}// if node is not present or node has no right child, x currently stores the successor
...
{// exclusively for path in search 
...
void lrot(struct node * x,struct node **root)
{
	...
	y->left = x;
	x->right=b;
	if(b!=nil)// sentinel has no parent
	b->parent=x;
	if(p!=NULL)
	{
		if(x==p->right)
...
void splice( struct node *b,struct node **root )//removal of a node with less than 2 children
{
	...
	if(b->color==1)// node to be deleted is red
	{
		p=b->parent;
	...
	{
		p=b->parent;
		if(b->left==nil&&b->right==nil)// "removing black token" from a sentinel ,it is done according to test cases 
		{
			if(b==p->left)
			{
				p->left=nil;
				s=p->right;
				if(s->color==1)//former sibling of deleted node is red: case 1 
				{
					s->color=0;
					p->color=1;
					lrot(p,root);
					s=p->right;
					// now it is in case 2, 3 or 4
					if(s->left->color==0 && s->right->color==0)//sibling is black and has 2 black children:case 2
					{
						s->color=1;
						blacken(p,root);	
					}
					else if(s->left->color==1&&s->right->color==0)//sibling is black with right child black and left child red:case 3
					{
						s->left->color=0;
						s->color=1;
						rrot(s,root);
					...
					else{//sibling is black with right child red: case 4
						s->right->color=0;
						s->color=p->color;
						p->color=0;
						lrot(p,root);
				...
				else if(s->left->color==0 && s->right->color==0)//sibling is black and has 2 black children:case 2
				{
					s->color=1;
					blacken(p,root);	
				}
				...
				}//sibling is black with right child red: case 4
				else{
					s->right->color=0;
					s->color=p->color;
					p->color=0;
			...
			else// mirror cases of cases stated above
			{
				p->right=nil;
				s=p->left;
				if(s->color==1)
		...
		else// concept of black token can be used if the deleted node has at least one non sntinel child 
		{
			if(b==p->left)
			{
				if(b->left==nil)
					...
					p->left=b->left;
				p->left->parent=p;
				blacken(p->left,root);// if it is a red node, it is colored black ,else removal of black node is done as explained in blacken() function
			}
			else
			{
				if(b->left==nil)
...
void blacken(struct node * a,struct node **root)
{
	...
	if(a==p->left)
	{
		s=p->right;// all cases in this block are similar to cases in the splice function with both children as sentinels
		if(s->color==1)// case 1
		{
			p->color=1;
			s->color=0;
	...
	else// mirror case of cases stated above
	{
		s=p->left;
		if(s->color==1)
		{
...
void delnode(struct node **root, struct node *a)
{
	...
	{
		splice(a,root);// root may be needed to be changed
	}
	else// two children
	{
	s=succ(a->var,*root);
